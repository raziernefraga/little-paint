      ...
      italicEnabled(false)
{
    ...
    connect(canvas, &CanvasWidget::colorPicked, this, &MainWindow::setColorFromEyedropper);
    connect(canvas, &CanvasWidget::outlineColorPicked, this, &MainWindow::setOutlineColorFromEyedropper);
    connect(canvas, &CanvasWidget::fillColorPicked, this, &MainWindow::setFillColorFromEyedropper);
...
void MainWindow::createActions() {
    ...
    savePngTransparent = new QAction("Salvar como PNG com transparência", this);
    connect(savePngTransparent, &QAction::triggered, this, [=]() {
        canvas->setExportTransparency(true);
        canvas->saveImage("imagem.png");
    });

    savePngVisible = new QAction("Salvar como PNG com fundo visível", this);
    connect(savePngVisible, &QAction::triggered, this, [=]() {
        canvas->setExportTransparency(false);
        canvas->saveImage("imagem.png");
    });

    saveJpg = new QAction("Salvar como JPG", this);
    connect(saveJpg, &QAction::triggered, this, [=]() {
        canvas->setExportTransparency(false);
        canvas->saveImage("imagem.jpg");
    });

    saveBmp = new QAction("Salvar como BMP", this);
    connect(saveBmp, &QAction::triggered, this, [=]() {
        canvas->setExportTransparency(false);
        canvas->saveImage("imagem.bmp");
    });
...
void MainWindow::createMenus() {
    ...
    fileMenu->addAction(savePngTransparent);
    fileMenu->addAction(savePngVisible);
    fileMenu->addAction(saveJpg);
    fileMenu->addAction(saveBmp);
...
void MainWindow::createToolbars() {
    ...
    styleBar->setObjectName("Style");

    outlineColorButton = new QPushButton(this);
    outlineColorButton->setText("Outline");
    outlineColorButton->setFixedSize(32, 32);
    outlineColorButton->setStyleSheet("background-color: black");

    fillColorButton = new QPushButton(this);
    fillColorButton->setText("Fill");
    fillColorButton->setFixedSize(32, 32);
    fillColorButton->setStyleSheet("background-color: white");

    styleBar->addWidget(outlineColorButton);
    styleBar->addWidget(fillColorButton);

    connect(outlineColorButton, &QPushButton::clicked, this, &MainWindow::changeOutlineColor);
...
void MainWindow::resizeCanvas() {
    bool ok;
    int width = QInputDialog::getInt(this, "New Width", "Enter new width:", canvas->canvasSize().width(), 10, 10000, 1, &ok);
    if (!ok) return;
    int height = QInputDialog::getInt(this, "New Height", "Enter new height:", canvas->canvasSize().height(), 10, 10000, 1, &ok);
    if (!ok) return;
    canvas->resizeCanvas(width, height);
}
...
void MainWindow::openPreferences() {
    ...
    layout->addWidget(new QLabel("Default Color:"));

    QPushButton *okButton = new QPushButton("OK");
    layout->addWidget(okButton);

    connect(okButton, &QPushButton::clicked, [&]() {
        thickness = thicknessBox->value();
...
void MainWindow::setOutlineColorFromEyedropper(const QColor &color) {
    currentOutlineColor = color;
    canvas->setOutlineColor(color);
    outlineColorButton->setStyleSheet(QString("background-color: %1").arg(color.name()));
}

void MainWindow::setFillColorFromEyedropper(const QColor &color) {
    currentFillColor = color;
